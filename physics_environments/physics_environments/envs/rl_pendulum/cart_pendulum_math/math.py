...
from physics_environments.envs.rl_pendulum.cart_pendulum_math.types import CartPendulumPhysicsConstants

class CartPendulumMath():
    """ This is a Mixin class for the CartPendulumPhysicsEnvironment, provding all the symbolic math calculations.
        The math was used from https://colab.research.google.com/drive/1tonlB7P0w4EZv2eC8PMP9zO-FzwBizb_#scrollTo=GwZi4egvspyT

        Inputs:
            constants:
                - simply provide an instance of CartPendulumPhysicsConstants
            initialize_math:
                - The math only needs to be instantiated to see math.summary, or wants to debug the function
                - The rest of the class is still useful as data holder, and with its vizualization methods.

        Note:
            - Initializing this class fully can take more than a minute for n>3
            - The slow Sympy functions like smp.simplify() do not support multiprocessing and are always executed on one core.
    """
    def __init__(self,
                 params             : Dict[str, Any] = CartPendulumPhysicsConstants(n=1),
                 initialize_math    : bool           = True):
        self.initialize_math = initialize_math


        if isinstance(params, CartPendulumPhysicsConstants):
            self._define_constants(constants_str2val = params.to_dict())
        else:
            self._define_constants(constants_str2val = dict(params)) # some lazy support fix to allow omegaconf.dictconfig.DictConfig from Stoix

        if self.initialize_math is True:
    ...
    def _define_constants(self,
                          constants_str2val : Dict[str, Any]
                          ) -> None:

        constants_str2val.update({'m_c' : None, 'mu_0' : None}) # two constants without effect on the simulation

        # Defining constant string-to-symbol mapping and symbol-to-value mapping here
        consts_s  = ['n', 'g', 'm_c']                           # n: number of rods, w: track width, w_c: cart width, m_c: cart mass (m_c has no effect inside simulation)
        ...
        mu_v      = [constants_str2val['mu_0'],    *constants_str2val['mu']] # --> [mu_0, mu_1, ..., mu_n]
    ...
    def _define_simulation_functions(self) -> None:
        ...
        sim_func_strings     += x_c_strings
        sim_func_expressions += x_c_functions
        ...
        T_trans_summands     = [smp.Rational(1,2)*self.constants['str_to_sym']["m_c"] * x_c_functions[1]**2]
    ...
    def _define_q_matrices_and_q_mappings(self) -> None:
    ...
    def _define_equations_and_matrices(self) -> None:
        ...
        t              = self.variables['t']
        L_func         = str2func['L']
        mu_strings     = [r"mu_0"] + [fr"mu_{n_}" for n_ in self.n_range]
        ...
        ddq_f                 = smp.lambdify([*theta_symbols, *dtheta_symbols, ddx_c_symbol, *self.ddqf_constants['sym_to_val']], [*ddq_sol_system], docstring_limit=None) # important to use only ddqf_constants; docstring_limit=None allows to copy the function implementations from the __doc__ string
